...
public class QueryBuilder<T, ID> extends StatementBuilder<T, ID> {
	...
	private void matchJoinedFields(JoinInfo joinInfo, QueryBuilder<?, ?> joinedQueryBuilder) throws SQLException {
		for (FieldType fieldType : tableInfo.getFieldTypes()) {
			...
			for(FieldType joinFieldType : joinedQueryBuilder.tableInfo.getFieldTypes()) {
				if (fieldType.isForeign() && foreignIdField.equals(joinFieldType)) {
					joinInfo.localField = fieldType;
					joinInfo.remoteField = foreignIdField;
					return;
				}
				if(joinFieldType.isForeign() && joinFieldType.getForeignIdField().equals(fieldType)) {
					joinInfo.localField = fieldType;
					joinInfo.remoteField = joinFieldType;
					return;
				}
			}
		}
