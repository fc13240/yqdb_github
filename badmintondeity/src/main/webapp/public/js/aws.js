var AWS =
{
	...
	ajax_post: function(formEl, processer, type) // 表单对象，用 jQuery 获取，回调函数名
	{
	    ...
	    {
	    	$('.modal-backdrop').removeClass('out, hide').addClass('in');
	    }
		...
		else
		{
	        ...
	        else
	        {
	        	switch (type)
	        	{
					...
					// Modify by wecenter
					case 'refound_processer':
						$('#request_form, .error_messag').hide();
	...
	dialog: function (type, data, callback)
	{
	    switch (type)
	    {
			...
			case 'recommend':
	 		...
	 		// Modify by wecenter
	 		case 'gonodataReference' :
	 			var template = Hogan.compile(AW_TEMPLATE.gonodataReference).render({
	 				'title': data.title
	 			});
	 		break;
	 		...
	 		// Modify by wecenter
	 		case 'refound' :
	 			var template = Hogan.compile(AW_TEMPLATE.refound).render({
	 				'title': data.title,
	 		...
	 		// Modify by wecenter
	 		case 'customerClient':
	 			var template = Hogan.compile(AW_TEMPLATE.customerClient).render({
	 				'title': data.title,
	 			});
	 		break;
	 		...
	 		// Modify by wecenter
	 		case 'dataDownload':
				var template = Hogan.compile(AW_TEMPLATE.dataDownload).render({
	 				'title': data.title,
	    ...
	    if (template)
	    {
	        ...
	        switch (type)
	        {
				...
				// Modify by wecenter
				case 'gonodataReference':
					$('.gonodataReference-box .mod-body ul').append(data.content);
				break;
			...
			{
				if (parseInt($('.customerClient-box .modal-body').scrollTop()) + 300 >= parseInt($('.customerClient-box .mod-body').innerHeight()))
				{
