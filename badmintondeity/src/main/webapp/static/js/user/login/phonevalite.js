...
$(function($){
	...
	$('#phonevalite_CptBtn').click(function(){
		if(checkphone()){
			$('#cptcusMobile').val($('#userno').val());
			alert($('#cptcusMobile').val());
			ajaxCommonSubmit('phonevaliteform', 'subok','/p2pstock/ajax/ajaxCpt_cptAction.action');
		}
	...
	//点击登录
	$('#phonevalite_Btn').click(function(){
		if(formchk()){
			$("#phonevaliteform").submit();
		}
	});
});
//发送手机验证码回调函数
function subok(msg){
	alert(msg);
	if($.trim(msg) == 0){
		time($('#phonevalite_CptBtn'),"120px","200px");
	}else if($.trim(msg) == 1){
		$('#lerror').text("发送失败请重试");
		$('#lerror').css("display", "block");
	}
