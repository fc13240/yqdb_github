		...
		var methods = {
			...
			addDates : function( dates, type ) {
				if(dates.length > 0) {
					...
					var len = this.multiDatesPicker.dates.picked.length;
					if(len == 1){
						var date = this.multiDatesPicker.dates.picked[0];
						$("#start_date").val(date.getFullYear()+"-"+(date.getMonth()+1)+"-"+date.getDate());
						$("#end_date").val($("#start_date").val());
					}else if(len == 2){
						var start_date = this.multiDatesPicker.dates.picked[0];
						$("#start_date").val(start_date.getFullYear()+"-"+(start_date.getMonth()+1)+"-"+start_date.getDate());
						var end_date = this.multiDatesPicker.dates.picked[1];
						$("#end_date").val(end_date.getFullYear()+"-"+(end_date.getMonth()+1)+"-"+end_date.getDate());
					}
					$("#inputShowDate").val($("#start_date").val()+" 至 "+$("#end_date").val());
				} else {
