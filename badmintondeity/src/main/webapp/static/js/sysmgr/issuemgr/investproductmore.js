function secAccChargeTyp(){
	var charges_type = $("#user_accruedcharges_type").val();
	if("0" == charges_type){
...
function addinfo(basic_product_id,basic_product_name,invest_product_id,basic_product_typename,launcher_accruedcharges_type,launcher_accruedcharges_amount,start_date,end_date){
	...
	if(basicarry.indexOf(basic_product_id) >= 0){
		...
		"<td  class='tbl_td_15'><a href='javascript:delinfo("+ basic_product_id  +","+launcher_accruedcharges_amount +")'>删除</a></td></tr>";
...
function saveinfo(){
	if( checkprdtname() && checkbind()){
		var fee = $("#issue_feetxt").val();
		if(confirm("发起本投资产品需要缴纳管理费"+fee+"元，确认发起吗？")){
			$('#saveform').submit();
		}
	}
}