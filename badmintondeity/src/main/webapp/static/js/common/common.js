...
function sendCpt(){
	...
	if(checkPhoneRule(mobile)){
		$.ajax({
			type : "post",// 使用get方法访问后台
			url : "/p2pstock/ajax/ajaxCpt_cptAction.action?phone="+ mobile,
			dataType : "json",
			success : function(resultMap) {
				if(resultMap.result==0){
					$("#cptBase").val(resultMap.cptno);
					$("#cptTime").val(resultMap.activetime);
					countdown();
				}else{
					alert("验证码发送失败");
				}				
...
// 冷却时间
function countdown() {
	$("#CptBtn").attr("disabled", "disabled");
	$("#CptBtn").val(wait+"秒后重新发送");
	wait--;
	if (wait >= 0) {
