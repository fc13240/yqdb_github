...
public class TreeMenuIntercept extends MethodFilterInterceptor {
    ...
    protected String doIntercept(ActionInvocation invocation) throws Exception {
        logger.debug("进入拦截器TreeMenuIntercept");
        MenuService menuService = new MenuServiceImpl();
        HttpServletRequest request = (HttpServletRequest) invocation.getInvocationContext().get(
                ServletActionContext.HTTP_REQUEST);
        // 获取当前menucode
        String menucode = request.getParameter("menucode");
        logger.debug("print menucode");
        logger.debug(menucode);
        // String menucodes =
        // (String)request.getSession().getAttribute("menusec");
        if (menucode == null) {
            if (request.getSession().getAttribute("menusec") == null) {
                menucode = "ACTIVITYPAGE";
            } else {
                menucode = request.getSession().getAttribute("menusec").toString();
            }
            //
